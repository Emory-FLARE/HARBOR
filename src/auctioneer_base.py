...
from .prompt_base_persona import PARSE_BID_INSTRUCTION
    ...
    def ask_for_bid(self, bid_round: int):
        ...
        if self.highest_bidder is None:
            if bid_round > 0:
                ...
                msg = f"Attention, bidders! {len(remaining_items)} item(s) left, they are: {', '.join(remaining_items)}.\n\nNow, please bid on {self.cur_item}, {self.cur_item.desc}. The starting price for bidding for {self.cur_item} is ${self.cur_item.price}. Anyone interested in this item?"
        else:
