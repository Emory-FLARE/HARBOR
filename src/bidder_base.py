...
from .prompt_base_persona import (
    AUCTION_HISTORY,
...
class Bidder(BaseModel):
    ...
    use_external_reason: bool = False
    is_2nd_tom: bool = False
    auction_hash: str = ''
    ...
    plan_instruct: str = ''
    profile_instruct: str = ''     # instruction for planning
    ...
    def _post_init(self):
        ...
        if self.is_profiler:
            self.system_message = SYSTEM_MESSAGE_PROFILER.format(
                name=self.name,
    ...
    def _parse_llm(self):
        if 'gpt-' in self.model_name:
            ...
            self.llm = ChatOpenAI(model="o1-preview", temperature=self.temperature, max_retries=30, request_timeout=1200)
        ...
        #     self.llm = OpenChat(model_name=f'models/{self.model_name}', temperature=self.temperature)
        elif 'rule' in self.model_name or 'human' in self.model_name:
            self.llm = None
        elif 'gemini' in self.model_name:
            self.llm = ChatGoogleGenerativeAI(model=self.model_name, temperature=self.temperature,  max_retries=30, timeout=1200, max_output_tokens=2048)
        else:
    ...
    def _run_llm_standalone(self, messages: list):
        ...
        with get_openai_callback() as cb:
            for i in range(6):
                try:
                    if "o1" not in self.model_name.lower():
                        input_token_num = self.llm.get_num_tokens_from_messages(messages)
                    if 'claude' in self.model_name:     # anthropic's claude
                        # print(type(self.llm))
                        # print([method for method in dir(self.llm) if callable(getattr(self.llm, method))]) max_tokens_to_sample=2048
                        result = self.llm(messages, max_tokens=2048)
                    ...
                    elif 'openchat' in self.model_name.lower():
                        result = self.llm()
                    elif "o1" in self.model_name.lower():
                        result = self.llm(messages, max_tokens=2048)
    ...
    def _get_cur_item(self, key=None, with_desc=False):
        if self.cur_item_id < len(self.items):
            if key is not None:
                return self.items[self.cur_item_id].__dict__[key]
            else:
                if with_desc:
                    return self.items[self.cur_item_id], self.items[self.cur_item_id].desc
    ...
    def _get_all_items(self, as_str=False):
        remain_items = self.items
        if as_str:
            return ', '.join([item.name for item in remain_items])
        else:
            return remain_items
        
    def format_external_reason_input(self, strategic_msg,  auctioneer_msg, cur_item):
        """
        Format input for the model.
        """
        if "Attention" not in auctioneer_msg:
            strategic_msg = strategic_msg.replace("round of bidding for this item", "round of bidding for "+cur_item.get_desc())
        user_message = strategic_msg.split("\n\n")[:-3]
        auctioneer_msg_list = user_message[:2]
        user_msg = user_message[2:]
        if "0" in self.name:
            auctioneer_msg_list.append("You are Bidder 0. You have 2 competitors Bidder 1 and Bidder 2.")
        elif "1" in self.name:
            auctioneer_msg_list.append("You are Bidder 1. You have 2 competitors Bidder 0 and Bidder 2.")
        else:
            auctioneer_msg_list.append("You are Bidder 2. You have 2 competitors Bidder 0 and Bidder 1.")
        user_msg.append("Analyze the situation at hand, and recommend if you should bid or withdraw.")
        auctioneer_msg_list.extend(user_msg)
        return "\n\n".join(auctioneer_msg_list)
    
    def external_strategic_planning(self, model, strategic_instruct):
        print("instruct: ")
        print(strategic_instruct)
        return "I have decided to bid more money..."
    ...
    def _get_items_value_str(self, items: List[Item]):
        ...
        for i, item in enumerate(items):
            ...
            _info = f"{i+1}. {item}, {item.desc}, starting price is ${item.price}. Your estimated value for this item is ${estimated_value}.\n"
            items_info += _info
    ...
    def learn_from_prev_auction(self, past_learnings, past_auction_log):
        if not self.enable_learning or 'rule' in self.model_name or 'human' in self.model_name:
            return ''
        if self.is_profiler:
            instruct_learn = INSTRUCT_LEARNING_TEMPLATE_PROFILER.format(
    ...
    def learn_long_mem(self, bidder_name, engagement_history, items, past_believe):
        instruct_long_mem = INSTRUCT_LONG_MEMORY.format(
            bidder_name = bidder_name,
            past_engagement = engagement_history,
            items_info = items, 
            other_bidder_preferences = past_believe,
        )

        result = self._run_llm_standalone([HumanMessage(content=instruct_long_mem)])
        self.dialogue_history += [
            HumanMessage(content=instruct_long_mem),
            AIMessage(content=result),
        ]
        self.llm_prompt_history.append({
            'messages': [{x.type: x.content} for x in [HumanMessage(content=instruct_long_mem)]],
            'result': result,
            'tag': 'longmem_0'
        })
        ...
        if self.long_term_profile != '':
            self.system_message += f"\n\nHere are some information on other bidders from previous auctions. You can use them to guide this auction:\n```\n{self.long_term_profile}\n```"
        return result
    ...
    def get_plan_instruct(self, items: List[Item]):
        self.items = items
        if self.is_profiler:
            plan_instruct = INSTRUCT_PLAN_TEMPLATE_PROFILER.format(
                bidder_name=self.name,       
                budget=self.budget, 
                item_num=len(items), 
                items_info=self._get_items_value_str(items), 
                desire_desc=DESIRE_DESC[self.desire],
                persona = self.persona, ### my injection
                learning_statement='' if not self.enable_learning else _LEARNING_STATEMENT_PROFILER
            )
        else:
    ...
    def init_plan(self, plan_instruct: str):
        ...
        result = self._run_llm_standalone(messages) ## API calling
    ...
    def get_second_tom_instruct(self, bidding_history: str):
        cur_item, item_desc = self._get_cur_item(with_desc=True)
        instruct = INSTRUCT_2TOM.format(
            cur_item=cur_item,
    ...
    def do_second_tom(self, instruct_tom:str):
        strategy_message = HumanMessage(content=instruct_tom)
        ...
        self.llm_prompt_history.append({
            'messages': [{x.type: x.content} for x in messages],
            'result': result,
            'tag': f'tom_{self.cur_item_id}'
        })
        self.second_tom = result
        return result
    
    def get_strategy_instruct(self, auctioneer_msg: str, second_tom=None):
        if not self.is_2nd_tom:
            if len(self.current_profile) < 1:
                instruct = INSTRUCT_STRATEGY_TEMPLATE.format(
                    auctioneer_msg=auctioneer_msg,
                    current_plan = extract_jsons_from_text(self.cur_plan),
                    prev_status=self._status_json_to_text(self.status_quo),
                    desire_desc = DESIRE_DESC[self.desire],
                )
            else:
                instruct = INSTRUCT_STRATEGY_TEMPLATE_WITH_PROFILE.format(
                    auctioneer_msg=auctioneer_msg,
                    current_plan = extract_jsons_from_text(self.cur_plan),
                    prev_status=self._status_json_to_text(self.status_quo),
                    currrent_profile = self.current_profile,
                    desire_desc = DESIRE_DESC[self.desire],
                )               
        else:
            instruct = INSTRUCT_STRATEGY_TEMPLATE_TOM.format(
                auctioneer_msg=auctioneer_msg,
                current_plan = extract_jsons_from_text(self.cur_plan),
                second_tom = second_tom,
                prev_status=self._status_json_to_text(self.status_quo),
                currrent_profile = self.current_profile,
                desire_desc = DESIRE_DESC[self.desire],
            )
    ...
    def strategic_planning(self, instruct_strategy:str):
        strategy_message = HumanMessage(content=instruct_strategy)
        messages = [SystemMessage(content=self.system_message)]
        ...
        self.llm_prompt_history.append({
            'messages': [{x.type: x.content} for x in messages],
            'result': result,
            'tag': f'strategy_{self.cur_item_id}'
        })
        return result
    ...
    def get_bid_instruct(self, auctioneer_msg: str, bid_round: int, strategic_reasoning:str=None):
        auctioneer_msg = auctioneer_msg.replace(self.name, f'You ({self.name})')
        if self.is_profiler:
            if strategic_reasoning:
                bid_instruct = INSTRUCT_BID_TEMPLATE_PROFILER.format(
                    auctioneer_msg=auctioneer_msg, 
                    bidder_name=self.name,
        ...
        if bid_round == 0:
            ...
            bid_instruct = f'Now, the auctioneer says: "{auctioneer_msg}". Here is some professional strategic bidding advice to help you make your decision: "{strategic_reasoning}"'
    ...
    def get_summarize_instruct(self, bidding_history: str, hammer_msg: str, win_lose_msg: str):
        if self.is_profiler:
            instruct = INSTRUCT_SUMMARIZE_TEMPLATE_PROFILER.format(
                cur_item=self._get_cur_item(), 
                bidding_history=bidding_history, 
    ...
    def get_replan_instruct(self, allow_profile=False):
        if self.is_profiler:
            if allow_profile:
                instruct = INSTRUCT_REPLAN_TEMPLATE_PROFILER.format(
                    ...
                    profile_other_bidder = self.current_profile,
                    bidder_name=self.name,
                    desire_desc=DESIRE_DESC[self.desire],
                    persona = self.persona,
                    learning_statement='' if not self.enable_learning else _LEARNING_STATEMENT
    ...
    def get_profile_instruct(self, bidding_history: str, target_bidder: str):
        cur_item, item_desc = self._get_cur_item(with_desc=True)
        if len(self.current_profile) < 1:
            instruct = INSTRUCT_PROFILE_TEMPLATE.format(
                cur_item=cur_item,
                house_desc = item_desc,
                bidding_history= bidding_history,
                bidder_name=self.name,
                target_bidder = target_bidder,
            )
        else:
            instruct = INSTRUCT_REPROFILE_TEMPLATE.format(
                cur_item=cur_item,
                house_desc = item_desc,
                bidding_history= bidding_history,
                old_predict=self.current_profile[target_bidder],
                bidder_name=self.name,
                target_bidder=target_bidder
            )       
        return instruct
    
    def profile(self, instruct_profile: str):
        profile_msg = HumanMessage(content=instruct_profile)
        messages = [SystemMessage(content=self.system_message)]
        
        messages.append(profile_msg)
        result = self._run_llm_standalone(messages)
        new_profile_dict = extract_jsons_from_text(result)[-1]
        cnt = 0
        while len(new_profile_dict) != 1 and cnt < 2:
            err_msg = 'Your response does not contain a JSON-format profile for bidders. Please revise your profling'
            print("profilling error happened!")
            print(result)
            messages += [
                AIMessage(content=result),
                HumanMessage(content=err_msg),
            ...
            cnt += 1
        bidder_name = list(new_profile_dict.keys())[0]
        self.current_profile[bidder_name] = new_profile_dict[bidder_name]

        self.llm_prompt_history.append({
            'messages': [{x.type: x.content} for x in [HumanMessage(content=instruct_profile)]],
            'result': result,
            'tag': f'profile_{self.cur_item_id}'
        })
...
def bidding_multithread(bidder_list: List[Bidder],  
    ...
    '''
    assert func_type in ['plan', 'bid', 'summarize', 'replan', "profile"]
    ...
    def run_once(i: int, bidder: Bidder, auctioneer_msg: str):
        try:
            ...
            if func_type == 'bid':
                ...
                print(auctioneer_msg)
                result = bidder.profile(instruct_profile=auctioneer_msg)
            else:
